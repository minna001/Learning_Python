...
def delete_command():
    ...
    tkinter.messagebox.askquestion("askquestion", "Are you sure?")
    tkinter.messagebox.showinfo("Success",selected_tuple()[0] + " " + "Deleted")

def update_command():
    backend.update(selected_tuple[0],title_text.get(),title_Author.get(),title_year.get(),title_isbn.get())
    tkinter.messagebox.askyesnocancel(title="Update?", message="Are You sure You want to update" + title_text.get() + " " + "?")
